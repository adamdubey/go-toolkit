...
func TestTools_RandomString(t *testing.T) {
	...
	if len(s) != 10 {
		t.Error("ERROR: RandomString returned an invalid string length!")
	}
}
...
func TestTools_CreateDirIfNotExist(t *testing.T) {
	...
	err := testTool.CreateDirIfNotExist("./testdata/myTestDir")
	if err != nil {
		t.Error(err)
	}
	...
	_ = os.Remove("./testdata/myTestDir")
}
...
var slugTests = []struct {
	...
	{name: "complex string", s: "HellO woRLd!!?123", expected: "hello-world-123", errorExpected: false},
	{name: "japanese string", s: "こんにちは世界", expected: "", errorExpected: true},
	{name: "japanese and roman characters string", s: "こんにちは世界 = hello world", expected: "hello-world", errorExpected: false},
}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTool.DownloadStaticFile(rr, req, "./testdata", "app-download-test.md", "test-success.md")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "108" {
		t.Error("wrong content length of", res.Header["Content-Length"][0])
	}

	if res.Header["Content-Disposition"][0] != "attachment; filename=\"test-success.md\"" {
...
var jsonTests = []struct {
	...
	{name: "two JSON files", json: `{ "foo": "1" }{"alpha": "delta"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
...
func TestToools_ReadJSON(t *testing.T) {
	var testTool Tools
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if !payload.Error {
		t.Error("error set to false in JSON, and it should be set to true")
	}
