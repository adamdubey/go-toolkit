...
// CreateDirIfNotExist creates a directory and all necessary parents, if not exist
...
// DownloadStaticFile downloads a file and attempts to force the browser to avoid displaying it in window
...
// ReadJSON tries to read req.body and converts from JSON to Go variable
...
// WriteJSON takes a response status code and data and writes JSON to client
...
// ErrorJSON takes an error and status code and sends a JSON error message
...
// PushJSONToRemote posts data to some URL as JSON and returns response, status code, and error if any
