...
func uploadFiles(w http.ResponseWriter, r *http.Request) {
	...
	for _, item := range files {
		out += fmt.Sprintf("Uploaded %s to the uploads directory, renamed %s\n", item.OriginalFileName, item.NewFileName)
	}
...
func uploadOneFile(w http.ResponseWriter, r *http.Request) {
	...
	_, _ = w.Write([]byte(fmt.Sprintf("Uploaded 1 file, %s, to the uploads directory", f.OriginalFileName)))
}
